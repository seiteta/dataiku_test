...
# Create a header containing column name, import data from the csv files, and remove the "instance weight" column
...
# Change the type of the columns that contain numeric values but are categorical
...
# Check whether columns contain numerical or categorical data
numerical_columns = []
categorical_columns = []
...
## Transform values from counting lists into percentage 
def autoformat(counts_list):
    counts_list = [float(element) * 100 / sum(counts_list) for element in counts_list]
    return counts_list
...
# Create an empty list used inside the loop
small_difference = []
...
for column in census_header[:-1]:
        ...
        if census_fifty.dtype != 'int64':
                ...
                # Choose which columns will not be use by the classifier because the difference between classes in inferior to a threshold (in percentage)
                threshold = 15
                for element in range(len(y1)):
                    if abs(y1[element] - y2[element]) < threshold:
...
clf = tree.DecisionTreeClassifier(min_samples_leaf=2000)
...
graph.write_pdf("income-tree.pdf")
...
## Step 2: Remove the columns that don't explain much
to_del = []
for index, element in enumerate(test_X_names):
    if test_X_names[index] in small_difference:
        to_del.append(index)
...
# Predict classes with the previously calculated tree
